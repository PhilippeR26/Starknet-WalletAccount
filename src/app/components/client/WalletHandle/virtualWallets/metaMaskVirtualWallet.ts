import { VirtualWallet } from "@/type/types"
...
const metaMaskVirtualWallet: VirtualWallet & Eip6963SupportedWallet = {
  id: "metamask",
  name: "MetaMask",
  ...
  async loadWallet(windowObject: Record<string, unknown>) {
    if (!metaMaskVirtualWallet.provider) {
      metaMaskVirtualWallet.provider = await detectMetamaskSupport(windowObject)
    }
    ...
    const wallet = new MetaMaskSnapWallet(metaMaskVirtualWallet.provider, "*")
    return wallet
  },
  async hasSupport(windowObject) {
    metaMaskVirtualWallet.provider = await detectMetamaskSupport(windowObject)
    return metaMaskVirtualWallet.provider !== null
  },
}
